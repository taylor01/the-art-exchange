...
namespace :migrate do
  desc "Extract blob key mappings from local development system"
  task extract_blob_mappings: :environment do
    puts "🔍 Extracting blob key mappings from local development system..."

    # Query all posters with attached images
    mappings_query = Poster.joins(:image_attachment, image_blob: :attachments)
                           .select(
                             'posters.id as poster_id',
                             'posters.name as poster_name',
                             'active_storage_blobs.key as blob_key',
    ...
    mappings = mappings_query.to_a
    puts "📊 Found #{mappings.length} poster-to-blob mappings"

    # Generate Ruby hash format for seed file
    mappings_hash = mappings.map do |mapping|
      {
        poster_id: mapping.poster_id,
        poster_name: mapping.poster_name,
    ...
    json_output_path = Rails.root.join("db", "poster_blob_mappings.json")
    File.write(json_output_path, JSON.pretty_generate(mappings_hash))
    puts "✅ JSON mappings written to: #{json_output_path}"

    # Write to Ruby file for seed integration
    ruby_output_path = Rails.root.join("db", "poster_blob_mappings.rb")
    File.write(ruby_output_path, generate_ruby_mappings(mappings_hash))
    puts "✅ Ruby mappings written to: #{ruby_output_path}"

    # Generate production migration rake task
    ...
    puts "✅ Production migration task written to: #{migration_task_path}"

    puts "\n📋 Summary:"
    puts "   Posters with images: #{mappings.length}"
    puts "   Total file size: #{(mappings.sum { |m| m.byte_size.to_i } / 1024.0 / 1024.0).round(2)} MB"
  ...
  def generate_ruby_mappings(mappings_hash)
    <<~RUBY
      # frozen_string_literal: true
      # Generated automatically from development database
      # Contains poster-to-blob mappings for production image migration

      POSTER_BLOB_MAPPINGS = #{mappings_hash.inspect}.freeze
    RUBY
  end
